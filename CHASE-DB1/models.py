...
def GCN_BR_RVASPP_Ghost_Mnet(input_size = (256,256,1)):
    def deaspp(x,lst):
    ...
    def GCN_BR(x,out_channel,kernel_size,ra=0.5):
        #GCN
        conv111 = Conv2D(out_channel*ra, [kernel_size,1], activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(x)
        conv111 = Conv2D(out_channel*ra, [1,kernel_size], activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv111)
        conv112 = Conv2D(out_channel*(1-ra), [kernel_size,1], activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv111)
        conv112 = Conv2D(out_channel*(1-ra), [1,kernel_size], activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv112)

        conv11 = concatenate([conv111,conv112], axis = 3)
        conv11 = BatchNormalization(axis=3)(conv11)

        conv121 = Conv2D(out_channel*ra, [1,kernel_size], activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(x)
        conv121 = Conv2D(out_channel*ra, [kernel_size,1], activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv121)
        conv122 = Conv2D(out_channel*(1-ra), [1,kernel_size], activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv121)
        conv122 = Conv2D(out_channel*(1-ra), [kernel_size,1], activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv122)       
        
        conv12 = concatenate([conv121,conv122], axis = 3)
        conv12 = BatchNormalization(axis=3)(conv12)

        conv1 = add([conv11,conv12])
        conv1 = BatchNormalization(axis=3)(conv1)
        #BR
        conv1b1 = Conv2D(out_channel*ra, kernel_size, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv1)
        conv1b2  = Conv2D(out_channel*(1-ra), kernel_size, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv1b1)
        conv1b = concatenate([conv1b1,conv1b2], axis = 3)
        conv1b = BatchNormalization(axis=3)(conv1b)

        conv2b1 = Conv2D(out_channel*ra, kernel_size, padding = 'same', kernel_initializer = 'he_normal')(conv1b)
        conv2b2  = Conv2D(out_channel*(1-ra), kernel_size, padding = 'same', kernel_initializer = 'he_normal')(conv2b1)
        conv2b = concatenate([conv2b1,conv2b2], axis = 3)
        conv2b = BatchNormalization(axis=3)(conv2b)

        conv1 = add([conv1,conv2b])
        conv1 = BatchNormalization(axis=3)(conv1)
        return conv1
    ...
    gb1 = GCN_BR(inputs,64,3)
        
    pool1 = MaxPooling2D(pool_size=(2, 2))(gb1)#32*32*64  
    ...
    gb2 = GCN_BR(is2,128,3)

    pool2 = MaxPooling2D(pool_size=(2, 2))(gb2)#16*16*128 
    ...
    gb3 = GCN_BR(is3,256,3)

    drop3 = Dropout(0.5)(gb3)
    ...
    conv61 = SeparableConv2D(128, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(merge6)#(None, 16, 16, 256) 
    conv62 = SeparableConv2D(128, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv61)#(None, 16, 16, 256) 
    conv6 = concatenate([conv61,conv62], axis = 3)#(None, 16, 16, 256) 
    ...
    #Channel Attention1
    CB = Reshape(target_shape=(32*32,128))(gb2)#N*C   
    CC = Reshape(target_shape=(128,32*32))(gb2)#C*N
    CS = Dot_layer([CC,CB])
    CS = Activation('softmax')(CS)
    CF = Dot_layer([CB,CS])
    ...
    conv71 = SeparableConv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(merge7)#(None, 32, 32, 128)
    conv72 = SeparableConv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv71)#(None, 32, 32, 128)
    conv7 = concatenate([conv71,conv72], axis = 3)#(None, 32, 32, 128)
    ...
    #Channel Attention1
    CB = Reshape(target_shape=(64*64,64))(gb1)#N*C   
    CC = Reshape(target_shape=(64,64*64))(gb1)#C*N
    # CS = Dot()([CC,CB])#C*C
    CS = Dot_layer([CC,CB])
    CS = Activation('softmax')(CS)
    # CF = Dot()([CB,CS])
    ...
    conv81 = SeparableConv2D(32, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(merge8)#(None, 64, 64, 64) 
    conv82 = SeparableConv2D(32, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv81)#(None, 64, 64, 64) 
    conv8 = concatenate([conv81,conv82], axis = 3)#(None, 64, 64, 64) 
...
def GCN_BR_RVASPP_Mnet(input_size = (256,256,1)):
    def deaspp(x,lst):
        x = Conv2D(256, [3,3], activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(x)
