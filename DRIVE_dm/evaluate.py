...
from extract_patches import hq
from extract_patches import get_data_testing
from extract_patches import get_data_testing_overlap

# from sy_post_processing import *
...
# dimension of the patches patches的规模
patch_height = 64
patch_width  = 64
#the stride in case output with average 以平均输出情况下的步幅
stride_height = 5
stride_width  = 5
...
#N full images to be predictedN个完整的图像要预测
Imgs_to_test = 2
#Grouping of the predicted images预测图像的分组
N_visual = 1
#====== average mode ===========平均模式
average_mode = True
...
if average_mode == True:
    patches_imgs_test, new_height, new_width, masks_test = get_data_testing_overlap(#测试的patch集、图片高度和宽度、标签集
...
best_last = 'best'
patches_imgs_test = np.einsum('klij->kijl', patches_imgs_test)#维度从（模块数，通道数，高度，宽度）->（模块数，高度，宽度，通道数）
...
# pred_imgs = post_processing(pred_imgs)#后处理
print ("Orig imgs shape: " +str(orig_imgs.shape))
print ("pred imgs shape: " +str(pred_imgs.shape))
print ("Gtruth imgs shape: " +str(gtruth_masks.shape))

# np.save('post_imgs',pred_imgs)
np.save('resutls', gtruth_masks)
...
y_scores,y_true = hq(pred_imgs,gtruth_masks)
