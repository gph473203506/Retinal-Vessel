...
def get_data_training(DRIVE_train_imgs_original,
    ...
    print("train_imgs："+str(train_imgs.shape))
    ...
    #extract the TRAINING patches from the full images从完整的图像中提取训练补丁
...
def get_data_testing(DRIVE_test_imgs_original, DRIVE_test_groudTruth, Imgs_to_test, patch_height, patch_width):
    ...
    #extract the TEST patches from the full images从完整的图像中提取测试补丁
    patches_imgs_test,N_h,N_w = extract_ordered(test_imgs,patch_height,patch_width)
    patches_masks_test,N_h,N_w = extract_ordered(test_masks,patch_height,patch_width)
    ...
    return patches_imgs_test, patches_masks_test,N_h,N_w




# Load the original data and return the extracted patches for testing加载原始数据并返回提取的补丁进行测试
# return the ground truth in its original shape 把GT还原成它原来的形状
def get_data_testing_overlap(DRIVE_test_imgs_original, DRIVE_test_groudTruth, Imgs_to_test, patch_height, patch_width, stride_height, stride_width):
    ...
    # test_imgs = my_PreProc(test_imgs_original)#预处理
    test_imgs = test_imgs_original
    test_masks = test_masks/255.
    #extend both images and masks so they can be divided exactly by the patches dimensions扩展图像和标记，这样它们就可以精确地按补丁大小划分
    test_imgs = test_imgs[0:Imgs_to_test,:,:,:]
    test_masks = test_masks[0:Imgs_to_test,:,:,:]
    ...
    print (patches_imgs_test.shape)
    print ("test PATCHES images range (min-max): " +str(np.min(patches_imgs_test)) +' - '+str(np.max(patches_imgs_test)))
    #返回的是用来测试的patch集patches_imgs_test、图片高度和宽度、标签集
...
def extract_random(full_imgs,full_masks, patch_h,patch_w, N_patches, inside=True):
    ...
    patch_per_img = int(N_patches/full_imgs.shape[0])  #N_patches equally divided in the full images每个图片划分的patch数
    print ("patches per full image: " +str(patch_per_img))
    iter_tot = 0   #iter over the total number of patches (N_patches)iter除以总补丁数
    for i in range(full_imgs.shape[0]):  #loop over the full images循环整个图像
        k=0
...
#check if the patch is fully contained in the FOV检查补丁是否完全包含在FOV中
...
#Divide all the full_imgs in pacthes 从测试图片中提取测试用的patch
...
def recompone_overlap(preds, img_h, img_w, stride_h, stride_w):
    #preds:训练后的patchs图片集。img_h:原始图片高度。img_w：原始图片宽度。stride_h：步幅高度。 stride_w：步幅宽度
...
def recompone(data,N_h,N_w):
    ...
    # print("patch_h："+str(patch_h)+'patch_w：'+str(patch_w)+'N_full_imgs：'+str(N_full_imgs)+',data.shape[1]：'+str(data.shape[1]))
